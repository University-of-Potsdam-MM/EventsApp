...
app = {
	...
	get: function(url){
		...
		if(app.requests[url] && !this.refreshing && app.cacheTimes[url] && Date.now() - app.cacheTimes[url] < 5 * 3600000) { //Alle 5 Stunden wird aktualisiert
        	...
        	$.getJSON(this.jsonUrl + url).done(function(d){ app.requests[url] = d; app.cacheTimes[url] = Date.now(); q.resolve(d);});
		}
		return q.promise;
	},
	...
	bindEvents:function(){
		...
		$(document).on('click', 'a', function(e){
			...
			if(href && href.indexOf('javascript:') == -1 && href.indexOf('http://') == -1 && href.indexOf('maps:') == -1 && href.indexOf('geo:') == -1) {
				$('.ui-btn-active', app.activePage()).removeClass('ui-btn-active');
