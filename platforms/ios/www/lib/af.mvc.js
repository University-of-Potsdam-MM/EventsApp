 ...
 * @Author Ian Maffett
 * @jQuery compatibility added by Martin Kapp 2014
 */
//mvc.js
...
(function($){
    ...
    $.mvc.app.prototype = {
        ...
        ready: function(fnc) {
            if(!this.loaded)  $(document).one("afmvc:loaded", fnc);// $(document).one("afmvc:loaded", fnc);
            else fnc();
        },
	...
	$.mvc.getTemplateID = function(url){
		return url.replace(/\//g, '-').replace(/\./g, '__');
	};
    $.mvc.getTemplateUrl = function(tmpl){
		return tmpl.replace(/\-/g, '/').replace(/__/g, '.');
	}
...
(function($){
	...
	$.mvc.controller.create = function(name, obj) {
        ...
        if(obj.hasOwnProperty("views") && (obj.views.length > 0 || Object.keys(obj.views).length) > 0) {
            ...
            for(var i in obj.views) {
                ...
                if(!viewsCache[shortName] && af("#" + $.mvc.getTemplateID(shortName)).length == 0) {
    ...
    $.mvc.controller.addView = function(path, controller, name) {
        $.get(path, function(data) {
           	var id = $.mvc.getTemplateID(name);
            $('body').append($("<script type='" + $.mvc._app._templateType + "' id='" + id + "'>" + data + "</script>"));
